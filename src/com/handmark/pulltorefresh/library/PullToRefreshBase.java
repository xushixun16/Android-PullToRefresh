...
public abstract class PullToRefreshBase<T extends View> extends LinearLayout implements IPullToRefresh<T> {
    ...
    /**
     * Checks whether is complete refresh.
     */
    private boolean mIsCompleteRefresh = false;
	...
	/**
	 * Setting footer refreshing
	 */
	final void setFooterRefreshing()
	{
	    if ((mMode.showFooterLoadingLayout()) && !isRefreshing())
	    {
	        mCurrentMode = Mode.PULL_FROM_END;
	        if (null != mOnRefreshListener) {
	            setState(State.REFRESHING, true);
	...
	final void setState(State state, final boolean... params) {
		...
		mIsCompleteRefresh = false;
		switch (mState) {
			case RESET:
			    mIsCompleteRefresh = true;
				onReset();
				break;
		...
		switch (getPullToRefreshScrollDirection()) {
			case VERTICAL:
				if (mState != State.RESET) {
					scrollTo(0, value);
				}else if (mCurrentMode == Mode.PULL_FROM_START) {
					scrollTo(0, value);
				}else {
				    if (!(getRefreshableView() instanceof ListView))
				    {
	                    getRefreshableView().bringToFront();
	                    scrollTo(0, value);
				    }
				}
				break;
	...
	private void init(Context context, AttributeSet attrs) {
		...
		mFooterLayout.setPullLabel(getContext().getString(R.string.pull_to_refresh_pull_up_label));
	...
	private void pullEvent() {
		...
		switch (mCurrentMode) {
			case PULL_FROM_END:
				...
				// TODO: forbidden the pull up event
				return;
//				break;
			case PULL_FROM_START:
			default:
	...
	private final void smoothScrollTo(int newScrollValue, long duration, long delayMillis,
		...
		if (oldScrollValue != newScrollValue) {
			if (null == mScrollAnimationInterpolator) {
				...
				mScrollAnimationInterpolator = new AccelerateDecelerateInterpolator();
			}
	...
	final class SmoothScrollRunnable implements Runnable {
		...
		public void run() {
			...
			if (mStartTime == -1) {
				...
				// just used for scroll view
				if (mIsCompleteRefresh && getRefreshableView() instanceof ScrollView)
				{
				    getRefreshableView().scrollBy(0, mScrollToY != mCurrentY ? (deltaY - mLastDeltaY) : (mScrollFromY - mLastDeltaY));
				    mLastDeltaY = mScrollToY != mCurrentY ? deltaY : mScrollFromY;
				}
			}
